...
export default function ProfilePage() {
  ...
  return (
    <div className="page-scroll scrollbar-none anim-in" style={{ paddingBottom: 100 }}>
      ...
      {/* ── Avatar + name header ── */}
      <div style={{ padding: "18px 16px 4px", display: "flex", alignItems: "flex-start", gap: 14 }}>
        <div style={{ position: "relative", flexShrink: 0 }}>
          <UserAvatar name={displayedName} size={76} avatarUrl={avatarUrl || undefined} />
          {editing && (
            <button
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {editing ? (
            ...
            <div style={{ fontSize: 20, fontWeight: 800, color: "var(--color-text)", marginBottom: 2 }}>{displayedName}</div>
          )}
          <div style={{ fontSize: 13, color: "var(--color-text-soft)" }}>@{profile?.username}</div>
          {memberSince && <div style={{ fontSize: 11, color: "var(--color-text-soft)", marginTop: 2 }}>Member since {memberSince}</div>}
      ...
      <div style={{ padding: "14px 16px 0" }}>
        ...
        {/* ── Stats ── */}
        <SectionCard style={{ display: "flex", justifyContent: "space-around", padding: "14px 8px" }}>
          {[
            [friendCount, "Connections", () => navigate("/connect")],
            [groups.length, "Groups", () => navigate("/groups")],
            [activities.length, "Activities", () => navigate("/meet")],
          ].map(([n, label, fn]) => (
            <div key={label} onClick={fn} style={{ textAlign: "center", cursor: "pointer" }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: "var(--color-primary)" }}>{n}</div>
              <div style={{ fontSize: 11, color: "var(--color-text-soft)", marginTop: 1 }}>{label}</div>
            </div>
          ))}
        </SectionCard>
          ...
          {editing ? (
            <>
              ...
              <Toggle
                ...
                sub="City/region only — never exact address"
              />
            </>
          ) : (
            <>
              <p style={{ fontSize: 14, color: "var(--color-text)", lineHeight: 1.65, margin: "0 0 10px" }}>
                {bio || <span style={{ color: "var(--color-text-soft)", fontStyle: "italic" }}>No bio yet</span>}
              </p>
        ...
        <SectionCard>
          ...
          <div style={{ fontSize: 11, color: "var(--color-text-soft)", lineHeight: 1.6, padding: "8px 0" }}>
            🔒 Mood, journal entries, and emotional check-ins are always private — they never appear on your profile.
          </div>
        </SectionCard>
        ...
        <SectionCard>
          ...
          <button onClick={() => navigate("/my-space")} style={{
            ...
            <span style={{ fontSize: 14, color: "var(--color-text)", fontFamily: "Rubik, sans-serif" }}>My Space (private)</span>
            <span className="material-symbols-outlined" style={{ fontSize: 16, color: "var(--color-text-soft)", marginLeft: "auto" }}>chevron_right</span>
          </button>
